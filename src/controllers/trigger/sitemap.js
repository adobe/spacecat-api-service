...
export const INITIAL_SITEMAP_SLACK_MESSAGE = '*SITEMAP DETECTION REPORT* for the *last week* :thread:';

/**
 * Triggers sitemap audit for websites based on the provided URL.
 *
...
export default async function trigger(context) {
  ...
  const slackContext = await getSlackContext({
    slackChannelId, url, message: INITIAL_SITEMAP_SLACK_MESSAGE, token, log,
  });
