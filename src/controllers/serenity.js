...
function SerenityController(context, log, env) {
  ...
  const createPrompts = async (ctx) => {
    try {
      ...
      // PROTOTYPE (SITES-47870 item 2): gate the per-item producing `source`
      // passthrough (main's LLMO-6556 mechanism) behind a FACS capability so only the
      // SR "Track" feature — not any config editor holding `llmo/can_configure` — may
      // assert a producing `source`. The decision is passed to the handler as an
      // explicit `allowAssertSource` option (NOT by rewriting the body), so the
      // capability gate can only be satisfied by this trusted controller path — a
      // future caller reaching the handler another way cannot enable the producer
      // surface with a bare body field (review should-fix). The capability name
      // (`<product>/can_track`) is registered in the MAC capability catalog.
      ...
      const result = auth.mode === 'subworkspace'
        ...
        : await handleCreatePrompts(
          ...
          writeDeadline,
          callerId,
          { allowAssertSource: assertSourceAllowed, orgId: ctx?.params?.spaceCatId },
        );
      return createResponse(result, 200);
    } catch (e) {
      return mapError(e, log);
