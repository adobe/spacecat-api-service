...
function ECA2Controller(context, log, env) {
  ...
  async function getECAWeeklySummary() {
    /* c8 ignore next 1 */
    ...
    log.info(`ECA WEEKLY-SUMMARY - yearInt: ${yearInt}, weekInt: ${weekInt}, monthInt: ${monthInt}`);

    const tableName = `${rumMetricsDatabase}.${rumMetricsCompactTable}`;
    const description = `fetch ECA2 Weekly Summary data db: ${rumMetricsDatabase}| siteKey: ${siteId} | year: ${year} | month: ${month} | week: ${week} } `;
    log.info(`ECA WEEKLY-SUMMARY - description: ${description}`);
    ...
    log.info(`ECA WEEKLY-SUMMARY - query: ${query}`);
