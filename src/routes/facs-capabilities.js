 ...
 * ```
 * {
 *   INTERNAL_ROUTES: ['METHOD /path', ...],   // routes excluded from FACS
 *   PRODUCTS_ROUTES: {
 *     LLMO: { 'METHOD /path': 'llmo/can_*', ... },
 *     ASO:  { 'METHOD /path': 'aso/...',     ... },
 *     ACO:  { ... },
 *   }
 * }
 ...
 * internal IMS orgs).
 *
 * `PRODUCTS_ROUTES` is keyed by uppercase SpaceCat product code. Each product owns a
 * complete map of every customer-facing route to its fully-qualified FACS permission
 * (e.g. `'llmo/can_view'`).
 *
...
const routeFacsCapabilities = {
  ...
  // (configure vs onboard vs deploy), not by HTTP verb.
    ...
    // ---- Onboard / integrations ----------------------------------------
      ...
      'GET /preflight/beta/jobs/:jobId': 'llmo/can_view',
