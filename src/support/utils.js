...
const X_PROMISE_TOKEN_HEADER = 'x-promise-token';
const BEARER_PREFIX = 'Bearer ';
...
function getHeaderCaseInsensitive(headers, headerName) {
  ...
  const headerEntry = Object.entries(headers).find(([name, value]) => (
    name.toLowerCase() === headerName && hasText(value)
  ));
  return headerEntry ? headerEntry[1].trim() : '';
}

function getPromiseTokenFromContext(context) {
  const dataPromiseToken = context?.data?.promiseToken?.promise_token;
  if (hasText(dataPromiseToken)) {
    return dataPromiseToken.trim();
  }

  const requestPromiseToken = context?.request?.headers?.get?.(X_PROMISE_TOKEN_HEADER);
  if (hasText(requestPromiseToken)) {
    return requestPromiseToken.trim();
  }

  return getHeaderCaseInsensitive(context?.pathInfo?.headers, X_PROMISE_TOKEN_HEADER);
}
...
/**
 * Resolve an access token for AEM API calls.
 * Precedence follows autofix-worker behavior:
 * 1) promise token (context.data.promiseToken.promise_token or x-promise-token)
 * 2) Authorization bearer token (pathInfo headers)
 * 3) Authorization bearer token (invocation event headers)
 *
 * @param {object} context - request context.
 * @param {object} [options] - optional injected dependencies for testing.
 * @param {Function} [options.exchangePromiseTokenFn] - promise token exchanger.
 * @param {Function} [options.getImsUserTokenFn] - bearer token extractor.
 * @returns {Promise<string>} Access token without Bearer prefix.
 * @throws {ErrorWithStatusCode} for auth failures.
 */
export async function resolveAemAccessToken(
  context,
  {
    exchangePromiseTokenFn = exchangePromiseToken,
    getImsUserTokenFn = getImsUserToken,
  } = {},
) {
  const promiseToken = getPromiseTokenFromContext(context);
  if (hasText(promiseToken)) {
    try {
      return await exchangePromiseTokenFn(context, promiseToken);
    } catch (e) {
      throw new ErrorWithStatusCode('Authentication failed with upstream IMS service', 401);
    }
  }

  try {
    return getImsUserTokenFn(context);
  } catch (e) {
    const invocationAuthorization = getHeaderCaseInsensitive(
      context?.invocation?.event?.headers,
      'authorization',
    );
    return extractBearerToken(invocationAuthorization);
  }
}
