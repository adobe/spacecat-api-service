...
export function createSerenityTransport({ env, imsToken }) {
  ...
  return {
     ...
     *
     * `resources` is REQUIRED by the contract (`handlers.createWorkspaceV2Form`) even though
     * every field inside it is optional — `{}` is how the spec expresses "no allocation".
     * Omitting it is accepted by the live gateway but refused by the spec-generated mock.
     *
     * @param {string} parentWorkspaceId
     * @param {string} title
     * @param {WorkspaceResources} resources - `{}` for no allocation.
     */
