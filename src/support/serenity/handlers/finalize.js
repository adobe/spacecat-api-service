...
export async function finalizeSerenityProjects(
  ...
  if (promptInputs.length > 0) {
    prompts = await handleCreatePrompts(
      ...
      // originValue is hardcoded AI, never derived from a principal: this
      // trigger fires on DRS/Brandalf generation completing, so every prompt
      // it creates is AI-generated by construction — there is no other
      // caller of this path.
