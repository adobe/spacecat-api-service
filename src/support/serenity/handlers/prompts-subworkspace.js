 ...
 * @param {boolean} [options.allowAssertSource] - PROTOTYPE (SITES-47870). Set true ONLY by
 *   the capability-checked controller (the FACS `<product>/can_track` gate). Mirrors the
 *   flat-mode twin {@link handleCreatePrompts}: when true AND `body.assertSource` opted in,
 *   the per-item producing `source` is honoured; otherwise it is stripped and the producer
 *   defaults to `config` (source-dimension.md §1 item 6).
 */
export async function handleCreatePromptsSubworkspace(
  ...
  // PROTOTYPE (SITES-47870): gate the per-item producing `source` — see the flat-mode
  // twin handleCreatePrompts. Honoured only with BOTH the controller's `allowAssertSource`
  // (the FACS `can_track` result) AND the request's `body.assertSource` opt-in.
  ...
  const results = await mapLimit(inputs, BULK_CREATE_CONCURRENCY, async (raw) => {
    ...
    // unless gated in — see the flat-mode twin handleCreatePrompts. Shallow copy, so the
    // caller's request body is never mutated.
    let item = raw;
