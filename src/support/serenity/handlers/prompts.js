 ...
 * @param {boolean} [options.allowAssertSource] - PROTOTYPE (SITES-47870). Set true ONLY
 *   by the capability-checked controller (the FACS `<product>/can_track` gate). When true
 *   AND the request opts in via `body.assertSource`, the per-item producing `source`
 *   (LLMO-6556's body passthrough) is honoured; otherwise it is stripped and the producer
 *   defaults to `config`, keeping `source` off the un-gated client write surface
 *   (source-dimension.md §1 item 6). Gating on the explicit option — not `body.assertSource`
 *   alone — means a future caller reaching this handler another way cannot enable the
 *   producer surface with a bare body field.
 */
export async function handleCreatePrompts(
  ...
  writeDeadline,
  callerId,
  { publish = true, orgId = null, allowAssertSource = false } = {},
) {
  ...
  // PROTOTYPE (SITES-47870): the per-item producing `source` is a gated surface. It is
  // honoured only when the trusted controller granted `allowAssertSource` (the FACS
  // `can_track` result) AND the request opted in via `body.assertSource`. Computed once
  // for the batch; enforced per item in the fan-out below (strip when not asserted).
  ...
  const results = await mapLimit(inputs, BULK_CREATE_CONCURRENCY, async (raw) => {
    // PROTOTYPE (SITES-47870): strip the per-item producing `source` BEFORE normalize
    // unless the request was gated in (see `assertSource` above), so an un-gated caller's
    // prompt falls back to the batch default (`config`) rather than stamping an asserted
    // producer. Stripped on a shallow copy — never mutate the caller's request body.
    let item = raw;
    if (!assertSource && raw && typeof raw === 'object') {
      item = { ...raw };
      delete item.source;
    }
    const { value: input, reason } = normalizePromptInput(item);
    if (!input) {
      return {
        skipped: {
