...
function coerceGapPromptsListTotal(value) {
  ...
  if (typeof value === 'string') {
    ...
    if (/[a-z]/i.test(t) && !/^[-+]?\d*\.?\d+e[-+]?\d+$/i.test(t)) {
      return undefined;
    }
...
function extractGapPromptsResponseTotal(body) {
  ...
  for (const block of [body.meta, body.pagination, body.page]) {
...
function normalizeGapPromptsBody(body) {
  ...
  const { offset: offsetRaw, limit: limitRaw } = body;
  let offset;
...
function normalizeSourceDomainRow(raw) {
  const sourcesCount = num(raw.sourcesCount ?? raw.sources_count);
  const mentions = num(raw.mentions ?? raw.overallMentions ?? raw.overall_mentions);
  const otRaw = raw.organicTraffic ?? raw.organic_traffic;
  const hasUpstreamOrganic = otRaw !== undefined && otRaw !== null && otRaw !== ''
  ...
  const row = {
    ...
    mentions,
    organic_traffic: hasUpstreamOrganic ? num(otRaw) : 0,
  };
  if (pe) {
    row.prompt_example = pe;
  }
  return row;
}
...
export function normalizeVisibilityV1SuccessfulBody(relPath, body) {
  ...
  if (relPath === '/competitors/gap-prompts') {
    return normalizeGapPromptsBody(body);
  }
  if (relPath === '/topics/research/source-domains') {
    return normalizeSourceDomainsBody(body);
  }
  return body;
}
