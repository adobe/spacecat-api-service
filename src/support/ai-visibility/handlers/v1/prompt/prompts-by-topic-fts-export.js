...
export async function handlePromptsByTopicFtsExport(sp, clients) {
  const engine = engineToLlm(sp.get('engine')) || LLM_ENUM.ALL;
  const country = resolveCountry(sp) || COUNTRY_ENUM.US;
  const { limit, offset } = parseLimitOffset(sp);
  let request;
  try {
    const listRequest = fromJson(PromptsByTopicFTSRequestSchema, {
      country,
      llm: engine,
      query: sp.get('query'),
      order: {
    ...
    request = fromJson(PromptsByTopicFTSExportRequestSchema, {
      request: listRequest,
      format: EXPORT_FILE_FORMAT_ENUM.CSV,
    }, PROTO_FROM_JSON);
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Invalid prompts by topic FTS export request';
    return { status: 400, body: { error: 'invalid_request', message } };
  }
  try {
    const response = await clients.promptClient.promptsByTopicFTSExport(request);
