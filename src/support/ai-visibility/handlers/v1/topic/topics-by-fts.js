...
export async function handleTopicsByFts(sp, clients) {
  ...
  try {
    const response = await clients.topicClient.topicsByFTS(request);
    const json = toJson(TopicsByFTSResponseSchema, response, PROTO_TO_JSON);
    return { status: 200, body: { data: json.topics ?? [] } };
  } catch (error) {
