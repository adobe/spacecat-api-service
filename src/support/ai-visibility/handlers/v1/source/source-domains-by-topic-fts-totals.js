...
import {
  SourceDomainsByTopicFTSTotalsRequestSchema,
  SourceDomainsByTopicFTSTotalsResponseSchema,
...
export async function handleSourceDomainsByTopicFtsTotals(sp, clients) {
  ...
  try {
    ...
    const message = error instanceof Error
      ? error.message
      : 'Invalid source domains by topic FTS totals request';
    return { status: 400, body: { error: 'invalid_request', message } };
  }
  try {
    const response = await clients.sourceClient.sourceDomainsByTopicFTSTotals(request);
    return {
      status: 200,
      body: toJson(SourceDomainsByTopicFTSTotalsResponseSchema, response, PROTO_TO_JSON),
    };
  } catch (error) {
