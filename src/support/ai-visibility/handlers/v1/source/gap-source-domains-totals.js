...
import {
  GapSourceDomainsTotalsRequestSchema,
...
export async function handleGapSourceDomainsTotals(sp, clients) {
  ...
  try {
    ...
    const message = error instanceof Error
      ? error.message
      : 'Invalid gap source domains totals request';
    return {
      status: 400,
  ...
  try {
    const totalsMessage = await clients.sourceClient.gapSourceDomainsTotals(totalsRequest);

    const totalsJson = /** @type {{ totals?: object[] }} */ (
      toJson(GapSourceDomainsTotalsResponseSchema, totalsMessage, PROTO_TO_JSON)
    );
