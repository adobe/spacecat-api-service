...
export async function handleBrandsByTopicFtsExport(sp, clients) {
  ...
  try {
    const listRequest = fromJson(BrandsByTopicFTSRequestSchema, {
      ...
      query: sp.get('query'),
      order: {
        by: sp.get('sortBy') || BRANDS_BY_TOPIC_FTS_REQUEST_ORDER_BY_ENUM.MENTIONS,
    ...
    const message = error instanceof Error ? error.message : 'Invalid brands by topic FTS export request';
    return { status: 400, body: { error: 'invalid_request', message } };
  }
  try {
    const response = await clients.brandClient.brandsByTopicFTSExport(request);
