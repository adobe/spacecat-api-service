 ...
 */
function getS3Key(imsOrgId) {
  return `${V2_CONFIG_PREFIX}/${imsOrgId}/config.json`;
}
 ...
 */
export async function getCustomerConfigV2FromS3(imsOrgId, s3Client, s3Bucket) {
  if (!hasText(imsOrgId)) {
 ...
 */
export async function saveCustomerConfigV2ToS3(imsOrgId, config, s3Client, s3Bucket) {
  if (!hasText(imsOrgId)) {
    throw new Error('IMS Org ID is required');
  }
 ...
 */
export async function customerConfigV2Exists(imsOrgId, s3Client, s3Bucket) {
  const config = await getCustomerConfigV2FromS3(imsOrgId, s3Client, s3Bucket);
  return config !== null;
}
