...
describe('s2sAuthWrapper integration', () => {
  ...
  it('succeeds when a route maps to an array of capabilities and the consumer holds one of them', async () => {
    const token = await createS2sToken();
    // GET .../serenity/brand-presence/stats maps to ['brand:read', 'organization:readAll'] -
    // a consumer only needs one of the two to pass.
    const suffix = '/v2/orgs/org-123/brands/brand-456/serenity/brand-presence/stats';
    ...
    const mockConsumer = {
      isRevoked: () => false,
      getStatus: () => 'ACTIVE',
      // Holds only the readAll alternative, not brand:read - still must pass.
