...
// by NATIVE_GLOBALS is restored to its module-load value. Lock it in here so
// a future refactor that breaks it surfaces immediately.
describe('setup-env root afterEach hook', () => {
  ...
  afterEach(() => {
    // Belt-and-suspenders: if a test in this file leaks, restore manually so
    // we don't poison the rest of the suite while testing the very hook that
    // would otherwise catch it.
    for (const prop of trackedGlobals) {
      if (globalThis[prop] !== snapshot[prop]) {
        const cur = globalThis[prop];
        if (cur && typeof cur.restore === 'function') {
          try {
            cur.restore();
          } catch {
            /* ignore */
  ...
  it('restores Date, queueMicrotask, setImmediate and timers together when sinon.useFakeTimers is left active', () => {
    sinon.useFakeTimers();
