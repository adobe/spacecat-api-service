...
describe('Customer Config Mapper', () => {
  describe('convertV1ToV2', () => {
    it('converts basic LLMO config to customer config', () => {
      ...
      expect(prompt.category.id).to.equal('cat-uuid-1');
      expect(prompt.category.name).to.equal('Photo Editing');
      expect(prompt.topic.id).to.equal('topic-uuid-1');
      expect(prompt.topic.name).to.equal('Photo Retouching');
    });
