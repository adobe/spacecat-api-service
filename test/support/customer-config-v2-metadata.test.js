...
describe('Customer Config V2 Metadata', () => {
  describe('mergeCustomerConfigV2', () => {
    ...
    // Additional edge case tests for metadata preservation
  ...
  describe('stripMetadata (exported for testing)', () => {
    it('should return null when input is null', async () => {
      ...
      const result = stripMetadata(null);
      expect(result).to.be.null;
    });

    it('should return undefined when input is undefined', async () => {
      const { stripMetadata } = await import('../../src/support/customer-config-v2-metadata.js');
      const result = stripMetadata(undefined);
      expect(result).to.be.undefined;
    });

    it('should return primitive values as-is', async () => {
      const { stripMetadata } = await import('../../src/support/customer-config-v2-metadata.js');
      expect(stripMetadata(42)).to.equal(42);
      expect(stripMetadata('string')).to.equal('string');
      expect(stripMetadata(true)).to.equal(true);
    });

    it('should strip metadata from objects', async () => {
      const { stripMetadata } = await import('../../src/support/customer-config-v2-metadata.js');
      const obj = {
        id: 'test-id',
        name: 'Test',
