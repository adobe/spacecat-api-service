...
describe('sandbox-audit-service helpers', () => {
  describe('normalizeAndValidateAuditTypes()', () => {
    ...
    it('returns error for invalid audit types', async () => {
      ...
      expect(body).to.have.property('message', 'Invalid audit types: invalid-audit, cwv. Supported types: meta-tags, product-metatags, commerce-product-enrichments, commerce-product-enrichments-yearly, alt-text');
    });
    ...
    it('returns error for mixed valid and invalid audit types', async () => {
      ...
      expect(result.error.status).to.equal(400);
      const body = await result.error.json();
      expect(body).to.have.property('message', 'Invalid audit types: invalid-audit. Supported types: meta-tags, product-metatags, commerce-product-enrichments, commerce-product-enrichments-yearly, alt-text');
    });
