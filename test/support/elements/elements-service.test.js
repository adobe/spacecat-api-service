/*
 * Copyright 2026 Adobe. All rights reserved.
 * This file is licensed to you under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License. You may obtain a copy
 * of the License at http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software distributed under
 * the License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR REPRESENTATIONS
 * OF ANY KIND, either express or implied. See the License for the specific language
 * governing permissions and limitations under the License.
 */

import { use, expect } from 'chai';
import chaiAsPromised from 'chai-as-promised';
import sinon from 'sinon';
import sinonChai from 'sinon-chai';
import { createElementsService } from '../../../src/support/elements/elements-service.js';
import { ELEMENT_IDS } from '../../../src/support/elements/element-ids.js';

use(chaiAsPromised);
use(sinonChai);

const RAW_BRANDS = {
  blocks: {
    value: [
      { value: 'Adobe' },
      { value: 'Nike' },
    ],
  },
};

const RAW_MARKETS = {
  blocks: {
    value: [
      { value: 'proj-1', label: 'US-en' },
      { value: 'proj-2', label: 'AU-en' },
    ],
  },
};

const RAW_TOPICS = {
  blocks: {
    value: [
      { value: 'topic:SEO' },
      { value: 'category:Firefly' },
      { value: 'intent:Informational' },
      { value: 'source:organic' },
    ],
  },
};

describe('createElementsService', () => {
  let transport;
  let service;

  beforeEach(() => {
    transport = { fetchElement: sinon.stub() };
    service = createElementsService(transport);
  });

  describe('getBrands', () => {
    it('calls fetchElement with BRANDS element ID', async () => {
      transport.fetchElement.resolves(RAW_BRANDS);
      await service.getBrands('ws-1', {});
      expect(transport.fetchElement).to.have.been.calledWith('ws-1', ELEMENT_IDS.BRANDS, sinon.match.object);
    });

    it('returns transformed filter dimensions', async () => {
      transport.fetchElement.resolves(RAW_BRANDS);
      const result = await service.getBrands('ws-1', {});
      expect(result).to.have.length(2);
      expect(result[0]).to.deep.include({ id: null, label: 'Adobe' });
    });

    it('resolves spacecat_brand_id when spacecatBrands are provided', async () => {
      transport.fetchElement.resolves(RAW_BRANDS);
      const spacecatBrands = [{ id: 'brand-1', name: 'Adobe' }];
      const result = await service.getBrands('ws-1', {}, spacecatBrands);
      expect(result[0].spacecat_brand_id).to.equal('brand-1');
    });

    it('defaults spacecatBrands to empty array', async () => {
      transport.fetchElement.resolves(RAW_BRANDS);
      const result = await service.getBrands('ws-1', {});
      expect(result[0].spacecat_brand_id).to.be.null;
    });

    it('propagates transport errors', async () => {
      transport.fetchElement.rejects(new Error('transport failure'));
      await expect(service.getBrands('ws-1', {})).to.be.rejectedWith('transport failure');
    });
  });

  describe('getMarkets', () => {
    it('calls fetchElement with MARKETS element ID', async () => {
      transport.fetchElement.resolves(RAW_MARKETS);
      await service.getMarkets('ws-1', {});
      expect(transport.fetchElement).to.have.been.calledWith('ws-1', ELEMENT_IDS.MARKETS, sinon.match.object);
    });

    it('returns transformed filter dimension regions', async () => {
      transport.fetchElement.resolves(RAW_MARKETS);
      const result = await service.getMarkets('ws-1', {});
      expect(result).to.have.length(2);
      expect(result[0]).to.deep.include({ id: 'US', label: 'US-en' });
    });

    it('enriches markets with BrandSemrushProject metadata', async () => {
      transport.fetchElement.resolves(RAW_MARKETS);
      const brandSemrushProjects = [
        {
          semrushProjectId: 'proj-1', brandId: 'brand-1', geoTargetId: 2840, languageCode: 'en',
        },
      ];
      const result = await service.getMarkets('ws-1', {}, brandSemrushProjects);
      expect(result[0].spacecat_brand_id).to.equal('brand-1');
      expect(result[0].geoTargetId).to.equal(2840);
    });

    it('defaults brandSemrushProjects to empty array', async () => {
      transport.fetchElement.resolves(RAW_MARKETS);
      const result = await service.getMarkets('ws-1', {});
      expect(result[0].spacecat_brand_id).to.be.null;
    });

    it('passes brand param in payload when provided', async () => {
      transport.fetchElement.resolves(RAW_MARKETS);
      await service.getMarkets('ws-1', { brand: 'Adobe' });
      const [, , payload] = transport.fetchElement.firstCall.args;
      expect(payload.filters.advanced.filters[0].val).to.equal('Adobe');
    });

    it('propagates transport errors', async () => {
      transport.fetchElement.rejects(new Error('transport failure'));
      await expect(service.getMarkets('ws-1', {})).to.be.rejectedWith('transport failure');
    });
  });

  describe('getTopics', () => {
    it('calls fetchElement with TOPICS element ID', async () => {
      transport.fetchElement.resolves(RAW_TOPICS);
      await service.getTopics('ws-1', {});
      expect(transport.fetchElement).to.have.been.calledWith('ws-1', ELEMENT_IDS.TOPICS, sinon.match.object);
    });

    it('returns transformed topic objects', async () => {
      transport.fetchElement.resolves(RAW_TOPICS);
      const result = await service.getTopics('ws-1', {});
      expect(result).to.have.length(4);
      expect(result[0]).to.deep.include({ value: 'topic:SEO', type: 'topic', name: 'SEO' });
    });

    it('includes projectId in payload when provided', async () => {
      transport.fetchElement.resolves(RAW_TOPICS);
      await service.getTopics('ws-1', { projectId: 'proj-uuid-123' });
      const [, , payload] = transport.fetchElement.firstCall.args;
      expect(payload.project_id).to.equal('proj-uuid-123');
    });

    it('propagates transport errors', async () => {
      transport.fetchElement.rejects(new Error('transport failure'));
      await expect(service.getTopics('ws-1', {})).to.be.rejectedWith('transport failure');
    });
  });

  describe('getUrlInspectorFilterDimensions', () => {
    beforeEach(() => {
      transport.fetchElement
        .withArgs('ws-1', ELEMENT_IDS.TOPICS, sinon.match.any)
        .resolves(RAW_TOPICS);
      transport.fetchElement
        .withArgs('ws-1', ELEMENT_IDS.BRANDS, sinon.match.any)
        .resolves(RAW_BRANDS);
      transport.fetchElement
        .withArgs('ws-1', ELEMENT_IDS.MARKETS, sinon.match.any)
        .resolves(RAW_MARKETS);
    });

    it('calls fetchElement three times (topics, brands, markets)', async () => {
      await service.getUrlInspectorFilterDimensions('ws-1', {});
      expect(transport.fetchElement).to.have.been.calledThrice;
    });

    it('fetches TOPICS, BRANDS and MARKETS in parallel', async () => {
      await service.getUrlInspectorFilterDimensions('ws-1', {});
      const calledIds = transport.fetchElement.getCalls().map((c) => c.args[1]);
      expect(calledIds).to.include(ELEMENT_IDS.TOPICS);
      expect(calledIds).to.include(ELEMENT_IDS.BRANDS);
      expect(calledIds).to.include(ELEMENT_IDS.MARKETS);
    });

    it('returns an object with brands, regions, topics, categories, page_intents, origins keys', async () => {
      const result = await service.getUrlInspectorFilterDimensions('ws-1', {});
      expect(result).to.have.all.keys(['brands', 'regions', 'topics', 'categories', 'page_intents', 'origins']);
    });

    it('brands contains filter dimensions for each brand', async () => {
      const result = await service.getUrlInspectorFilterDimensions('ws-1', {});
      expect(result.brands).to.have.length(2);
      expect(result.brands[0]).to.deep.include({ id: null, label: 'Adobe' });
    });

    it('regions contains transformed markets', async () => {
      const result = await service.getUrlInspectorFilterDimensions('ws-1', {});
      expect(result.regions).to.have.length(2);
      expect(result.regions[0].label).to.equal('US-en');
    });

    it('topics contains only topic:-prefixed entries', async () => {
      const result = await service.getUrlInspectorFilterDimensions('ws-1', {});
      expect(result.topics).to.deep.equal([{ id: null, label: 'SEO' }]);
    });

    it('categories contains only category:-prefixed entries', async () => {
      const result = await service.getUrlInspectorFilterDimensions('ws-1', {});
      expect(result.categories).to.deep.equal([{ id: null, label: 'Firefly' }]);
    });

    it('page_intents contains only intent:-prefixed entries with uppercased id', async () => {
      const result = await service.getUrlInspectorFilterDimensions('ws-1', {});
      expect(result.page_intents).to.deep.equal([{ id: 'INFORMATIONAL', label: 'Informational' }]);
    });

    it('origins contains only source:-prefixed entries', async () => {
      const result = await service.getUrlInspectorFilterDimensions('ws-1', {});
      expect(result.origins).to.deep.equal([{ id: 'organic', label: 'organic' }]);
    });

    it('resolves spacecat_brand_id on brands when spacecatBrands are provided', async () => {
      const spacecatBrands = [{ id: 'brand-1', name: 'Adobe' }];
      const result = await service.getUrlInspectorFilterDimensions('ws-1', {}, spacecatBrands);
      const adobeBrand = result.brands.find((b) => b.label === 'Adobe');
      expect(adobeBrand.spacecat_brand_id).to.equal('brand-1');
    });

    it('uses empty payload for markets fetch (no brand filter)', async () => {
      await service.getUrlInspectorFilterDimensions('ws-1', { brand: 'Adobe' });
      const marketsCall = transport.fetchElement.getCalls()
        .find((c) => c.args[1] === ELEMENT_IDS.MARKETS);
      expect(marketsCall.args[2]).to.not.have.nested.property('filters.advanced');
    });

    it('propagates transport errors', async () => {
      transport.fetchElement.withArgs('ws-1', ELEMENT_IDS.TOPICS, sinon.match.any)
        .rejects(new Error('upstream failure'));
      await expect(service.getUrlInspectorFilterDimensions('ws-1', {}))
        .to.be.rejectedWith('upstream failure');
    });
  });
});
