...
describe('kpi-headlines definitions', () => {
  ...
  describe('buildKpiHeadlinePayload', () => {
    ...
    it('pushes the category tag onto CBF_tags as-is when category is given', () => {
      const payload = buildKpiHeadlinePayload({
  ...
  describe('buildSourceVisibilityPayload', () => {
    ...
    it('pushes the category tag onto CBF_tags as-is when category is given', () => {
      const payload = buildSourceVisibilityPayload({
        brandUrls: ['lovesac.com'], startDate: '2026-06-25', endDate: '2026-07-24', category: 'category__Firefly',
      });
      expect(payload.filters.advanced.filters).to.deep.include({
        op: 'eq', val: 'category__Firefly', col: 'CBF_tags',
      });
