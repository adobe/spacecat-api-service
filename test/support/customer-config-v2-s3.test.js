...
describe('Customer Config V2 S3', () => {
  ...
  const imsOrgId = '1234567890ABCDEF@AdobeOrg';
  const mockConfig = {
    customer: {
      customerName: 'Adobe',
      imsOrgID: imsOrgId,
  ...
  describe('getCustomerConfigV2FromS3', () => {
    it('fetches config from S3 successfully', async () => {
      ...
      const result = await getCustomerConfigV2FromS3(imsOrgId, s3ClientStub, s3Bucket);
      ...
      expect(command.input.Key).to.equal(`customer-config-v2/${imsOrgId}/config.json`);
    });
    ...
    it('throws error for other S3 errors', async () => {
      ...
      await expect(
        getCustomerConfigV2FromS3(imsOrgId, s3ClientStub, s3Bucket),
      ).to.be.rejectedWith('S3 error');
    });

    it('throws error if IMS Org ID is missing', async () => {
      await expect(
        getCustomerConfigV2FromS3('', s3ClientStub, s3Bucket),
      ).to.be.rejectedWith('IMS Org ID is required');
    });

    it('throws error if S3 client is missing', async () => {
      await expect(
        getCustomerConfigV2FromS3(imsOrgId, null, s3Bucket),
      ).to.be.rejectedWith('S3 client and bucket are required');
    });

    it('throws error if bucket is missing', async () => {
      await expect(
        getCustomerConfigV2FromS3(imsOrgId, s3ClientStub, ''),
      ).to.be.rejectedWith('S3 client and bucket are required');
    });
  });
  ...
  describe('saveCustomerConfigV2ToS3', () => {
    it('saves config to S3 successfully', async () => {
      ...
      await saveCustomerConfigV2ToS3(imsOrgId, mockConfig, s3ClientStub, s3Bucket);
    ...
    it('throws error if config is missing', async () => {
      await expect(
        saveCustomerConfigV2ToS3(imsOrgId, null, s3ClientStub, s3Bucket),
      ).to.be.rejectedWith('Config is required');
    });

    it('throws error if S3 client is missing', async () => {
      await expect(
        saveCustomerConfigV2ToS3(imsOrgId, mockConfig, null, s3Bucket),
      ).to.be.rejectedWith('S3 client and bucket are required');
    });

    it('throws error if bucket is missing', async () => {
      await expect(
        saveCustomerConfigV2ToS3(imsOrgId, mockConfig, s3ClientStub, ''),
      ).to.be.rejectedWith('S3 client and bucket are required');
    });
  });
  ...
  describe('customerConfigV2Exists', () => {
    it('returns true if config exists', async () => {
      ...
      const exists = await customerConfigV2Exists(imsOrgId, s3ClientStub, s3Bucket);
