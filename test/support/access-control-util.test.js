...
describe('Access Control Util', () => {
  ...
  describe('hasAdminGrant', () => {
    ...
    it('returns reason=not-s2s when context has no s2sConsumer', async () => {
      const ctx = buildContext({ s2sConsumer: null });
      const util = AccessControlUtil.fromContext(ctx);
      const result = await util.hasAdminGrant('CREATE_SITE');
      expect(result.allowed).to.be.false;
      expect(result.reason).to.equal('not-s2s');
      expect(ctx.dataAccess.Consumer.findByClientIdAndImsOrgId).to.not.have.been.called;
    });

    it('returns reason=not-found when DB returns null', async () => {
      const ctx = buildContext({ s2sConsumer: makeS2SConsumer(), fresh: null });
      const util = AccessControlUtil.fromContext(ctx);
      const result = await util.hasAdminGrant('CREATE_SITE');
      expect(result.allowed).to.be.false;
    ...
    it('returns reason=revoked when consumer isRevoked', async () => {
      const fresh = makeFresh({ revoked: true });
      const ctx = buildContext({ s2sConsumer: makeS2SConsumer(), fresh });
      const util = AccessControlUtil.fromContext(ctx);
      const result = await util.hasAdminGrant('CREATE_SITE');
    ...
    it('returns reason=not-active when SUSPENDED', async () => {
      const fresh = makeFresh({ status: 'SUSPENDED' });
      const ctx = buildContext({ s2sConsumer: makeS2SConsumer(), fresh });
      const util = AccessControlUtil.fromContext(ctx);
      const result = await util.hasAdminGrant('CREATE_SITE');
    ...
    it('returns reason=missing-grant when adminGrants is null/undefined', async () => {
      const fresh = makeFresh({ adminGrants: undefined });
      const ctx = buildContext({ s2sConsumer: makeS2SConsumer(), fresh });
      const util = AccessControlUtil.fromContext(ctx);
      const result = await util.hasAdminGrant('CREATE_SITE');
      expect(result.allowed).to.be.false;
      expect(result.reason).to.equal('missing-grant');
      expect(result.consumerId).to.equal('consumer-id-1');
    });

    it('returns reason=missing-grant when key is absent from adminGrants', async () => {
      const fresh = makeFresh({ adminGrants: {} });
      const ctx = buildContext({ s2sConsumer: makeS2SConsumer(), fresh });
      const util = AccessControlUtil.fromContext(ctx);
      const result = await util.hasAdminGrant('CREATE_SITE');
      expect(result.allowed).to.be.false;
    ...
    it('returns reason=missing-grant when value is false', async () => {
      const fresh = makeFresh({ adminGrants: { CREATE_SITE: false } });
      const ctx = buildContext({ s2sConsumer: makeS2SConsumer(), fresh });
      const util = AccessControlUtil.fromContext(ctx);
      const result = await util.hasAdminGrant('CREATE_SITE');
      expect(result.allowed).to.be.false;
    ...
    it('returns allowed=true when adminGrants has operationKey: true', async () => {
      const fresh = makeFresh({ adminGrants: { CREATE_SITE: true } });
      const ctx = buildContext({ s2sConsumer: makeS2SConsumer(), fresh });
      const util = AccessControlUtil.fromContext(ctx);
      const result = await util.hasAdminGrant('CREATE_SITE');
      expect(result.allowed).to.be.true;
    ...
    it('uses identity from s2sConsumer for DB lookup', async () => {
      const fresh = makeFresh({ adminGrants: { CREATE_SITE: true } });
      const consumer = makeS2SConsumer('client-X', 'BBB222222222222222222222@AdobeOrg');
      const ctx = buildContext({ s2sConsumer: consumer, fresh });
      const util = AccessControlUtil.fromContext(ctx);
      await util.hasAdminGrant('CREATE_SITE');
      expect(ctx.dataAccess.Consumer.findByClientIdAndImsOrgId).to.have.been.calledOnceWith(
        'client-X',
        'BBB222222222222222222222@AdobeOrg',
      );
