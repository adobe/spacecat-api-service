...
describe('handlers/prompts.js — handleCreatePrompts', () => {
  ...
  // batch a single populate-then-publish. classifyPromptType is passed as the
  // 7th positional arg (undefined here) and the option object as the 8th.
  it('creates the prompt but does NOT publish when { publish: false }', async () => {
    ...
    }, fakeLog(), undefined, { publish: false });
...
describe('handlers/prompts.js — origin derivation (origin-dimension.md §3)', () => {
  ...
  it('update preserves the stored ai origin and never re-derives it', async () => {
    ...
    expect(transport.updatePromptTagsByIds).to.have.been.calledOnceWithExactly(
      ...
      [{
        id: 'ai-prompt',
        references: [TAG_IDS.categoryRunningShoes, TAG_IDS.originAi, TAG_IDS.typeBranded],
        replace: true,
      }],
    );
  });
