...
describe('handlers/prompts.js — handleCreatePrompts', () => {
  ...
  // The per-item producing `source` (main's LLMO-6556 body passthrough) is honoured
  // ONLY when the trusted controller granted `allowAssertSource` (the FACS `can_track`
  // gate) AND the request opted in via `body.assertSource`. Positional args after
  // body/log: classifyPromptType (7th), env (8th), writeDeadline (9th), callerId (10th),
  // then the options object as the 11th.
  describe('handleCreatePrompts source passthrough (SITES-47870 Track wiring)', () => {
    ...
    it('assertSource: honours the per-item producing `source` (semrush, not the config default)', async () => {
      ...
      }, fakeLog(), undefined, undefined, undefined, 'test-caller', { allowAssertSource: true });
    ...
    it('denied caller: body opts in but allowAssertSource:false keeps source closed (config)', async () => {
      ...
      }, fakeLog(), undefined, undefined, undefined, 'test-caller', { allowAssertSource: false });
