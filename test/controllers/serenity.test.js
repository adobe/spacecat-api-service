...
describe('SerenityController', () => {
  ...
  describe('defensive branch coverage', () => {
    ...
    // `options.allowAssertSource` (the options object — the 11th positional arg, index
    // 10, after the LLMO-6289 `callerId`) — NOT by rewriting the body. FAIL-CLOSED:
    // allowed only when the caller holds `<product>/can_track` or is an admin, never
    // merely because FACS is off.
    const trackData = () => ({
      assertSource: true,
