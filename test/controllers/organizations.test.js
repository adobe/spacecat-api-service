...
describe('Organizations Controller', () => {
  ...
  describe('GET /organizations/by-ims-org-id - S2S readAll capability', () => {
    ...
    function makeFreshConsumer({
      id = 'consumer-id-1', capabilities = ['organization:readAll'], status = 'ACTIVE', revoked = false,
    } = {}) {
      return {
        getId: () => id,
        getCapabilities: () => capabilities,
    ...
    it('grants a cross-tenant S2S consumer holding organization:readAll (bypasses org check)', async () => {
      sandbox.stub(AccessControlUtil.prototype, 'hasAccess').resolves(false);
      context.s2sConsumer = makeS2SConsumer();
      context.invocation = { id: 'req-bio-1' };
      mockDataAccess.Consumer.findByClientIdAndImsOrgId
        .resolves(makeFreshConsumer({ capabilities: ['organization:readAll'] }));

      const result = await organizationsController.getByImsOrgID(
        { params: { imsOrgId: IMS_ORG_ID }, ...context },
      );
      const organization = await result.json();

      expect(result.status).to.equal(200);
      expect(organization).to.have.property('imsOrgId', IMS_ORG_ID);
      expect(context.log.info).to.have.been.calledWithMatch(
        /\[s2s-readall\] GET \/organizations\/by-ims-org-id granted clientId=svc-1 consumerId=consumer-id-1 capability=organization:readAll requestId=req-bio-1/,
      );
    });

    it('still allows a tenant-scoped S2S consumer with only organization:read for its own org', async () => {
      sandbox.stub(AccessControlUtil.prototype, 'hasAccess').resolves(true);
      context.s2sConsumer = makeS2SConsumer();
      mockDataAccess.Consumer.findByClientIdAndImsOrgId
        .resolves(makeFreshConsumer({ capabilities: ['organization:read'] }));
    ...
    it('denies a cross-tenant S2S consumer without readAll', async () => {
      sandbox.stub(AccessControlUtil.prototype, 'hasAccess').resolves(false);
      context.s2sConsumer = makeS2SConsumer();
      context.invocation = { id: 'req-bio-deny' };
      mockDataAccess.Consumer.findByClientIdAndImsOrgId
        .resolves(makeFreshConsumer({ capabilities: ['organization:read'] }));
      ...
      expect(error).to.have.property('message', 'Only users belonging to the organization can view it');
      expect(context.log.info).to.have.been.calledWithMatch(
        /\[acl\] Denied GET \/organizations\/by-ims-org-id - reason=missing-capability clientId=svc-1 consumerId=consumer-id-1 requestId=req-bio-deny/,
      );
    });
