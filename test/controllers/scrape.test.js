...
describe('ScrapeJobController tests', () => {
  ...
  describe('createScrapeJob', () => {
    ...
    it('should reject when no scrapequeues are defined', async () => {
  ...
  describe('getScrapeJobProgress', () => {
    it('should respond with an expected progress response', async () => {
      baseContext.dataAccess.ScrapeJob.getScrapeJobProgress = sandbox.stub().resolves([
        createScrapeJob({
          ...exampleJob,
        }),
      ]);

      baseContext.dataAccess.ScrapeJob.findById = sandbox.stub().resolves(createScrapeJob({
        ...exampleJob,
      }));

      // only need to provide enough import url data to satisfy the import-supervisor, no need
      // for all the other properties of a ScrapeUrl object.
      baseContext.dataAccess.ScrapeUrl.allByScrapeJobId = sandbox.stub().resolves([
        ...
        // as no process will flip a ScrapeUrl status to running at this time, therefore
  ...
  describe('getScrapeJobResult', () => {
    ...
    it('should fail to fetch the scrape job result for a running job', async () => {
