...
describe('Sites Audits Controller', () => {
  ...
  it('updates multiple sites with multiple audits  and returns their responses', async () => {
    mockDataAccess.getSiteByBaseURL.withArgs('https://site1.com').resolves(sites[0]);
    mockDataAccess.getSiteByBaseURL.withArgs('https://site2.com').resolves(sites[1]);
  ...
  describe('bad request errors', () => {
    it('returns bad request when baseURL is not provided', async () => {
      ...
      checkBadRequestFailure(response, error, 'Base URL is required');
    });

    it('returns bad request when baseURL is empty', async () => {
      const requestData = [
  ...
  describe('misc errors', () => {
    ...
    it('return 500 when site cannot be updated with empty error message', async () => {
      mockDataAccess.getSiteByBaseURL.withArgs('https://site1.com').resolves(SiteDto.fromJson({
        id: 'site1', baseURL: 'https://site1.com', deliveryType: 'aem_edge',
      }));
      mockDataAccess.updateConfiguration.rejects(new Error());
      ...
      expect(response.status).to.equal(500);
      expect(error).to.have.property('message', 'Failed to enable/disable audits for all provided sites');
    });
  });
});
