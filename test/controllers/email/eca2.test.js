...
import ECA2Controller from '../../../src/controllers/email/eca2.js';
...
describe('ECA2Controller', () => {
  ...
  describe('getECAWeeklySummary', () => {
    it('returns 404 if site not found', async () => {
      mockContext.dataAccess.Site.findById.resolves(null);
      const controller = ECA2Controller(mockContext, mockLog, mockEnv);
      const res = await controller.getECAWeeklySummary();
    ...
    it('uses correct database and table names from env', async () => {
      ...
      const controller = ECA2Controller(mockContext, mockLog, mockEnv);
      await controller.getECAWeeklySummary();
