...
describe('LlmoController', () => {
  ...
  describe('onboardCustomer', () => {
    ...
    it('should return 400 for invalid region value (LLMO-4683)', async () => {
      const LlmoControllerOnboard = await esmock('../../../src/controllers/llmo/llmo.js', {
        '../../../src/controllers/llmo/llmo-onboarding.js': {
          validateSiteNotOnboarded: validateSiteNotOnboardedStub,
          performLlmoOnboarding: performLlmoOnboardingStub,
      ...
      const invalidRegionContext = {
        ...onboardingContext,
        data: { domain: 'example.com', brandName: 'Test Brand', region: 'usa' },
      };

      const result = await testController.onboardCustomer(invalidRegionContext);

      expect(result.status).to.equal(400);
      expect(performLlmoOnboardingStub).to.not.have.been.called;
    });

    it('should forward region to performLlmoOnboarding when supplied (LLMO-4683)', async () => {
      const LlmoControllerOnboard = await esmock('../../../src/controllers/llmo/llmo.js', {
        '../../../src/controllers/llmo/llmo-onboarding.js': {
          validateSiteNotOnboarded: validateSiteNotOnboardedStub,
          performLlmoOnboarding: performLlmoOnboardingStub,
      ...
      const ctxWithRegion = {
        ...onboardingContext,
        data: { ...onboardingContext.data, region: 'IN' },
      };

      const result = await testController.onboardCustomer(ctxWithRegion);
  ...
  describe('createOrUpdateEdgeConfig', () => {
    ...
    it('should skip opt-in notification (and avoid org-members fetch) when cdnProvider is aem-cs-fastly', async () => {
      const notifyOptInStub = sinon.stub().resolves({ sent: true });
      ...
      mockTokowakaClient.fetchMetaconfig.resolves(null);
      mockTokowakaClient.createMetaconfig.resolves(newMetaconfig);
      // S3 returns aem-cs-fastly — caller in llmo.js short-circuits before
      // calling notifyOptInIfNeeded so we never reach the org-members fetch
      // or the email send.
      readConfigStub.resolves({
        config: { cdnBucketConfig: { cdnProvider: 'aem-cs-fastly' } },
        exists: true,
        version: 'v1',
      });

      const LlmoControllerSkip = await esmock('../../../src/controllers/llmo/llmo.js', {
        '../../../src/controllers/llmo/llmo-config-metadata.js': {
          updateModifiedByDetails: updateModifiedByDetailsStub,
        },
        '../../../src/controllers/llmo/cdn-opt-in-notification.js': {
      ...
      const controllerSkip = LlmoControllerSkip({
        ...mockContext,
        env: { ...mockEnv },
      });

      const result = await controllerSkip.createOrUpdateEdgeConfig(edgeConfigContext);
  ...
  describe('getEdgeOptimizeInstallerUrl', () => {
    let installerContext;
    let getSignedUrlStub;

    beforeEach(() => {
      getSignedUrlStub = sinon.stub().resolves('https://llmo-edgeoptimize-cf-template.s3.us-east-1.amazonaws.com/edgeoptimize-cloudfront-installer.yaml?X-Amz-Signature=abc');
      // The handler trims the api key — seed a value with surrounding whitespace
      // to assert trimming.
      mockTokowakaClient.fetchMetaconfig.resolves({ apiKeys: ['  eo-secret-key  \n'] });
      installerContext = {
        ...mockContext,
        params: { siteId: TEST_SITE_ID },
        env: {},
    ...
    it('returns a quick-create URL prefilling SiteHost + EdgeOptimizeApiKey (but not DistributionId)', async () => {
      const result = await controller.getEdgeOptimizeInstallerUrl(installerContext);

      expect(result.status).to.equal(200);
      const body = await result.json();
      expect(body.quickCreateUrl).to.include('stacks/quickcreate');
      expect(body.quickCreateUrl).to.include('templateURL=');
      // siteHost is www.example.com (calculateForwardedHost on https://www.example.com)
      expect(body.quickCreateUrl).to.include('param_SiteHost=www.example.com');
      // api key is trimmed (no leading/trailing whitespace or newline survives)
      expect(body.quickCreateUrl).to.include('param_EdgeOptimizeApiKey=eo-secret-key');
      // DistributionId + DefaultOriginId are intentionally left unset (customer fills them)
      expect(body.quickCreateUrl).to.not.include('param_DistributionId');
      expect(body.quickCreateUrl).to.not.include('param_DefaultOriginId');
      expect(body.siteHost).to.equal('www.example.com');
      expect(body.presignTtlSeconds).to.equal(3600);
      expect(getSignedUrlStub.calledOnce).to.equal(true);
    });
    ...
    it('returns 400 when the site has no Edge Optimize API key', async () => {
      ...
      expect(result.status).to.equal(400);
      const body = await result.json();
      expect(body.message).to.include('no Edge Optimize API key');
    });

    it('returns 400 when template hosting is not configured (no S3 client)', async () => {
      const result = await controller.getEdgeOptimizeInstallerUrl({ ...installerContext, s3: {} });
    ...
    it('returns 403 when the user lacks access to the site', async () => {
      ...
      const result = await deniedController.getEdgeOptimizeInstallerUrl(installerContext);
    ...
    it('returns 403 when the user is not an LLMO administrator', async () => {
      ...
      const result = await LlmoControllerNoAdmin(mockContext)
        .getEdgeOptimizeInstallerUrl(installerContext);
  ...
  describe('getEdgeOptimizeOrigins', () => {
    let originsContext;
    ...
    it('returns the origins and hasEdgeOptimizeOrigin false when none match', async () => {
      const result = await controller.getEdgeOptimizeOrigins(originsContext);
    ...
    it('returns 400 for an invalid account id', async () => {
      const result = await controller.getEdgeOptimizeOrigins({ ...originsContext, data: { ...originsContext.data, accountId: '123' } });

      expect(result.status).to.equal(400);
    });

    it('returns 400 when the external id is missing', async () => {
      const result = await controller.getEdgeOptimizeOrigins({ ...originsContext, data: { accountId: '120569600543', distributionId: 'E2EXAMPLE123' } });

      expect(result.status).to.equal(400);
    });

    it('returns 400 when the distributionId is missing', async () => {
      const result = await controller.getEdgeOptimizeOrigins({ ...originsContext, data: { accountId: '120569600543', externalId: 'ext' } });
    ...
    it('returns 400 when the AWS call fails', async () => {
      ...
      expect(result.status).to.equal(400);
      const body = await result.json();
      expect(body.message).to.include('GetDistributionConfig failed');
    });
    ...
    it('returns 403 when the user lacks access to the site', async () => {
      ...
      const result = await deniedController.getEdgeOptimizeOrigins(originsContext);
    ...
    it('returns 403 when the user is not an LLMO administrator', async () => {
      ...
      const controllerNoAdmin = LlmoControllerNoAdmin(mockContext);
      const result = await controllerNoAdmin.getEdgeOptimizeOrigins(originsContext);

      expect(result.status).to.equal(403);
    });
  });

  describe('getEdgeOptimizeBehaviors', () => {
    ...
    it('returns the default behavior plus ordered cache behaviors', async () => {
      const result = await controller.getEdgeOptimizeBehaviors(behaviorsContext);
    ...
    it('returns 400 for an invalid account id', async () => {
      const result = await controller.getEdgeOptimizeBehaviors({ ...behaviorsContext, data: { ...behaviorsContext.data, accountId: '123' } });

      expect(result.status).to.equal(400);
    });

    it('returns 400 when the external id is missing', async () => {
      const result = await controller.getEdgeOptimizeBehaviors({ ...behaviorsContext, data: { accountId: '120569600543', distributionId: 'E2EXAMPLE123' } });

      expect(result.status).to.equal(400);
    });

    it('returns 400 when the distributionId is missing', async () => {
      const result = await controller.getEdgeOptimizeBehaviors({ ...behaviorsContext, data: { accountId: '120569600543', externalId: 'ext' } });
    ...
    it('returns 403 when the user lacks access to the site', async () => {
      ...
      const result = await deniedController.getEdgeOptimizeBehaviors(behaviorsContext);
  ...
  describe('deployEdgeOptimize', () => {
    ...
    it('returns 400 when the orchestrator throws', async () => {
      ...
      expect(result.status).to.equal(400);
      const body = await result.json();
      expect(body.message).to.include('assume failed');
    });
