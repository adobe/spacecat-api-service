...
describe('llmo-agentic-traffic', () => {
  ...
  describe('createAgenticTrafficByCategoryHandler', () => {
    ...
    it('substitutes Uncategorized for null category_name', async () => {
      const client = createMockClient({
        rpc_agentic_traffic_by_category: {
          data: [{ category_name: null, total_hits: 100 }],
          error: null,
        },
      });
      const ctx = makeContext({ client });
    ...
    it('returns status data', async () => {
      const client = createMockClient({
        rpc_agentic_traffic_by_status: {
          data: [{ http_status: 200, total_hits: 900 }, { http_status: 404, total_hits: 50 }],
          error: null,
        },
      });
      const ctx = makeContext({ client });
