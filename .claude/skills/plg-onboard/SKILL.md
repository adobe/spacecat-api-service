...
3. Interpret the response:
   ...
   | `200` with `status: "REJECTED"` | Terminal — provisioning refused, check `waitlistReason` | Most common auto-reject: the domain **requires authentication (login/SSO)** — ASO can't audit login-gated sites and there's no self-service fix, so it's rejected outright at the onboarding gate (`reason` includes `requires authentication`). Don't retry; the front door must be publicly crawlable |
