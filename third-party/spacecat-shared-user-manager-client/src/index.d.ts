...
/**
 * A fully-typed User Manager client over every operation in the generated `paths`.
 ...
 */
export type SerenityUserManagerApiClient = Client<paths>;

export interface SerenityUserManagerApiClientOptions {
  /**
   * Base URL of the User Manager API — the origin of `SEMRUSH_PROJECTS_BASE_URL` (the host is
   * shared with Project Engine), or the Counterfact mock's origin for E2E / local dev. Only
   * `protocol//host` is used; the client appends the fixed `/enterprise/users/api` prefix itself.
   */
  baseUrl: string;
  /**
 ...
 */
export declare function createSerenityUserManagerApiClient(
  options: SerenityUserManagerApiClientOptions,
