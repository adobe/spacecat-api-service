...
Single entry point for all URL Inspector HTTP APIs backed by mysticat-data-service (PostgREST). These endpoints power the **URL Inspector** dashboard (Owned URLs, Trending URLs, Domains tabs, plus domain and URL drill-downs, and the top-of-page filter dropdowns).
...
Deep-dive docs: [stats](url-inspector-stats-api.md), [owned-urls](url-inspector-owned-urls-api.md), [trending-urls](url-inspector-trending-urls-api.md), [cited-domains](url-inspector-cited-domains-api.md), [domain-urls](url-inspector-domain-urls-api.md), [url-prompts](url-inspector-url-prompts-api.md), [filter-dimensions](url-inspector-filter-dimensions-api.md).
...
| 7 | GET | `/org/:spaceCatId/brands/{all\|:brandId}/brand-presence/url-inspector/filter-dimensions` | Distinct categories / regions / content types in the site's data — option universe for the top-of-page filter dropdowns. | `siteId` | `startDate`, `endDate`, `platform`. `:brandId` / category / region **not** applied. | [url-inspector-filter-dimensions-api.md](url-inspector-filter-dimensions-api.md) |
...
| Store | Used by | Why |
|-------|---------|-----|
| `url_inspector_domain_stats` (summary table, keyed by `site_id, execution_date, model, hostname, content_type`) | `stats`, `cited-domains`, `filter-dimensions` | 100× faster than raw tables on large sites. Carries pre-aggregated `unique_prompts`, `unique_urls`, `citation_count`, plus `categories TEXT[]` and `regions TEXT[]`. **No `brand_id`** — that's why brand filtering is not available on these endpoints. |
...
- `model` / `platform` — when present, validated against the `llmo_execution_model` enum (`chatgpt`, `gemini`, `claude`, etc.) via `map_llmo_execution_model_input()`. When absent, the RPC applies **no** model filter (the SQL pattern is `v_platform IS NULL OR model = v_platform`). The `filter-dimensions` RPC additionally collapses the literal string `'all'` (case-insensitive) and empty/whitespace strings to NULL *before* consulting the alias map, so the dropdowns' "All models" sentinel does not silently narrow to ChatGPT-Paid via the legacy `'all' → 'chatgpt-paid'` alias (LLMO-4525 review fix).
...
```json
{
  "categories":    [{ "id": "Acrobat", "label": "Acrobat" }, { "id": "Analytics", "label": "Analytics" }],
  "regions":       [{ "id": "DE", "label": "DE" }, { "id": "GB", "label": "GB" }, { "id": "US", "label": "US" }],
  "content_types": [{ "id": "earned", "label": "earned" }, { "id": "owned", "label": "owned" }, { "id": "social", "label": "social" }]
}
```

All three arrays are alphabetically sorted (guaranteed via `jsonb_agg(... ORDER BY ...)` in the RPC). Empty / whitespace / NULL elements in upstream `categories[]` / `regions[]` are filtered out.
...
Filter-dimensions endpoint (LLMO-4525):
- mysticat-data-service: [#461](https://github.com/adobe/mysticat-data-service/pull/461)
- spacecat-api-service: [#2269](https://github.com/adobe/spacecat-api-service/pull/2269)
- project-elmo-ui: [#1599](https://github.com/adobe/project-elmo-ui/pull/1599)
