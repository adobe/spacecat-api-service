...
| 2 | GET | `/org/:spaceCatId/brands/{all\|:brandId}/brand-presence/weeks` | Returns ISO weeks (`YYYY-Wnn`) with calendar start/end for week pickers. Calls `rpc_brand_presence_execution_date_range` on `brand_presence_executions` and generates weeks from the date range. | <ul><li><code>model</code> : <code>gemini</code> (optional)</li><li><code>siteId</code> : <code>c2473d89-e997-458d-a86d-b4096649c12b</code> (optional)</li></ul> | [§2](#2-weeks) | [brand-presence-weeks-api.md](brand-presence-weeks-api.md) |
