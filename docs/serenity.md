...
- **The sub-workspace shell's fate depends on `SERENITY_ALLOW_WORKSPACE_DELETE`** (unset — off — in every deployed env today). This is the SAME flag that gates the test-cleanup/net-zero-smoke workspace delete; decommission reuses it rather than minting a second flag for the same underlying capability (LLMO-6189). With the flag **off**: deactivate empties every project and releases as much of the allocation as a transfer can move — which, for a FULL release, is **nothing** (a transfer that sets a dimension to 0 is a silently-ignored no-op against the Semrush gateway, confirmed by a live probe). The empty shell remains in the parent family, its allocation permanently stranded against the parent pool, until an operator flips the flag or Semrush CS deprovisions it manually. This is logged loudly (not as a false success) so the stranding is observable. With the flag **on**: deactivate deletes the (already project-emptied) workspace outright, genuinely returning its full carve to the parent pool. A future activate allocates a fresh sub-workspace either way.
