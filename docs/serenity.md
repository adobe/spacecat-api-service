...
| PATCH | `/serenity/tags/:tagId` | Rename and/or re-parent a tag by its upstream id. `name` is the full `<dimension>:<value>` string for a root, or a bare value for a child. `parentId`: an id RE-PARENTS, explicit `null` PROMOTES a child to root, omitted preserves the current parent (the proxy re-sends a child's current parent itself — omission is only safe for a root). Re-parent is validated (same-dimension root, no self-parenting, no demoting a populated root); a root rename cannot cross dimensions. | `updateSerenityTag` |
