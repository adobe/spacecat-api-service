...
| POST | `/serenity/prompts/bulk-tags` | Freeze the current matching prompt ids and enqueue an assign/remove set operation. Optional `Idempotency-Key` replays the same canonical request for 24 hours; use a new key to retry a terminal partial result. Every worker attempt retries the project-wide publish, including a no-op retry. | `bulkTagSerenityPrompts` |
