...
| GET | `/serenity/tags/search?geoTargetId=&languageCode=&q=&limit=&cursor=` | Search the plain `tag` dimension (never `category`/other dimensions) for names/paths containing `q` (NFKC-normalized, case-insensitive), ranked exact > prefix > substring > path. Reads a fresh, cacheless, complete tag tree per request (fails closed — `503` — rather than ever returning a partial/inconsistent result) and returns an opaque, HMAC-signed `cursor` for the next page (`null` once exhausted). Requires `SERENITY_TAG_SEARCH_CURSOR_SECRET`; without it every call fails closed with `503 tagSearchUnavailable`. | `searchSerenityTags` |
