...
| POST | `/serenity/tags` | Create/resolve a tag on one slice; body is `{ type, name, geoTargetId, languageCode, parentId? }`. `type` is `category`/`topic`/`tag` (open — customer-authored, `parentId` nests a 1-level bare-named child under a same-dimension root) or `source`/`intent`/`type` (closed — `name` must match the fixed enum; resolve-before-create, idempotent, `parentId` not allowed; response is `200 { ..., created }` not `201`). `parentId` on an open create is validated: it must reference an existing ROOT of the SAME dimension. | `createSerenityTag` |
| PATCH | `/serenity/tags/:tagId` | Rename and/or re-parent a tag by its upstream id. `name` is the full `<dimension>:<value>` string for a root, or a bare value for a child. `parentId`: an id RE-PARENTS, explicit `null` PROMOTES a child to root, omitted preserves the current parent (the proxy re-sends a child's current parent itself — omission is only safe for a root). On promote-to-root the bare child name is automatically **re-prefixed with its inherited dimension** (e.g. `Sneakers` → `category:Sneakers`), since a root must carry a `<dimension>:` prefix — so the returned `tag` differs from the `name` sent. Re-parent is validated (same-dimension root, no self-parenting, no demoting a populated root); a root rename cannot cross dimensions. | `updateSerenityTag` |
