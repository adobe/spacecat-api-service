...
5. **Cross-repo conflict contract (with companion plan).** The optimistic-lock `409` relies on the data-service RPC raising `SQLSTATE 40001` with the current version in the error `DETAIL`. The controller maps `error.code === '40001'` → `409 { message, currentVersion: Number(error.details) }`. Both plans pin this token.
